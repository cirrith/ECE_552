...
/				*Forward to decode for branch logic when the source register matches the register in Memory, avoids a two cycle stall
/					Should only do it when instruction is writing to register and it is not a memory read
/
/		INPUTS: 
...
/				*Branch_Forward_Exe - Need to stall for Branch, data needed is in execute currently
/				*Branch_Forward_Mem - For stall logic, whethere there is fowarding from memory to decode or not
/				*Branch_Data [15:0] - Data for decode stage
/
/		INTERNAL:
...
module Forward_Logic(Reg_1_Src, Reg_2_Src, Write_Reg_Mem, Write_Reg_WB, Reg_Write_Mem, Reg_Write_WB, ALU_Result_Mem, WB, A_Forward, B_Forward, Forward_Mem, A_Forward_Data, B_Forward_Data);
	...
	//output 			Branch_Forward_Exe;
	//output 			Branch_Forward_Mem;
	//output [15:0]	Branch_Data;
	...
	//assign Branch_Forward_Exe = 
	//assign Branch_Forward_Mem = 
