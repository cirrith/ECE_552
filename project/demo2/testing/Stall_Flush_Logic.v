...
/					*-Branch logic uses a register that is currently in memory
/		INPUTS:
...
/				*Branch_Stall - Need to stall for Branch, data needed is in execute currently
/				
/		OUTPUTS: 
...
/				*Stall_Branch - If there should be a stall due to a branch instruction
********************************************************************************************************/
module Stall_Flush_Logic(PC_Sel, halt_Dec, halt_Exe, halt_Mem, halt_WB, Forward_Mem, Mem_Read_Mem, Branch_Stall, Flush_IFDE, Flush_DEEX, Flush_EXME, Flush_MEWB, Stall_Fetch, Stall_IFDE, Stall_DEEX, Stall_EXME, Stall_MEWB);
	...
	assign Stall_Branch = Branch_Stall & Mem_Read_Mem; //If not reading from memory, should 
	
	assign Flush_IFDE = PC_Sel | halt_Dec | halt_Exe | halt_Mem | halt_WB;   //Flush everything behind a halt or for a PC redirect
	assign Flush_DEEX = halt_Exe | halt_Mem | halt_WB;   //Flush everything behind a halt
