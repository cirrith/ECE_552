...
/					0 = Pass through A
/					1 = Pass through B
/				PC_Code_Dec - Determine how the jump/branch PC is calculated
...
/					0 = Pass through A
/					1 = Pass through B
/				PC_Code_Exe - Determine how the jump/branch PC is calculated
/					0 - Jump/Branch (PC + 2 + Immediate)
/					1 - Jump (RS + Immediate)
...
//Execute
	...
	dff_pipe pc_code	 		(.clk(clk), .rst(rst), .Flush(Flush), .Stall(Stall), .d(PC_Code_Dec),		.q(PC_Code_Exe));
